...
class AppDelegate: UIResponder, UIApplicationDelegate, MessagingDelegate, UNUserNotificationCenterDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        let carListController = CarListController(collectionViewLayout: layout)
        let navController = CustomNavigationController(rootViewController: carListController)
    ...
    private func attemptRegisterForNotification(application: UIApplication)
    {
        ...
        UNUserNotificationCenter.current().requestAuthorization(options: options) { (granted, error) in
            ...
            if let error = error
            {
                NSLog("Failed to request auth: \(error)")
            }
